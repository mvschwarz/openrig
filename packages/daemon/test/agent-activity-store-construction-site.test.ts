...
// `resolveOccupantGeneration` silently falls to the legacy clock-only path and DISABLES the
...
describe("AgentActivityStore — single production construction site (W2a-1)", () => {
  ...
  it("has exactly one production construction site, and it injects resolveOccupantGeneration", () => {
    const sites: Array<{ file: string; injectsResolver: boolean }> = [];

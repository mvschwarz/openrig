...
FACT | **Contact with the operator is open by default — the route-through-a-router doctrine is retired** [amended 2026-08-27 per operator ruling R23, release-0.5.5 SHAPING.md]. ANY seat may contact the operator directly for escalations; orchestrators and PMs may also send updates and informational items they judge the operator would want. What remains true: **the operator is very likely not there and is certainly not watching your screen** — so prefer the durable escalation surfaces for anything that must survive their absence, and expect etiquette to become policy only if a real problem appears.
...
RECIPE | **"Is that seat stuck, or thinking?"** | The row face derives it now (S04): `pickup` on every projection — working / stalled-after-claim (evidence named) / parked / unclaimed — plus `rig view show pickup` board-wide; the old capture+`claimedAt`+`queue transitions` join is RETIRED as the first move. `rig capture` stays the liveness check only. **Liveness is not health** — a pane can render while nothing progresses.
